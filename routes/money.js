...
router.post("/listm",function(req,res,next){
      ...
      if(err){
        ...
        if(secret == config.secret){
          if(userkey.supper==1){
            ...
            Money.find({_id_company: idcompany},function(err,money){
            if(err) 
                res.json({success:false, message:"Lỗi kết nối!"});
              res.json(money);
          });
...
router.post("/category",function(req,res,next){
    ...
    User.findOne({_id:userid},{_id_company: 1,supper:1},function(err,userkey){
      if(err){
        ...
        if(secret == config.secret){
          if(userkey.supper==1){
            Money.find({type:key},function(err,money){
            if(err) 
                res.json({success:false, message:"Lỗi kết nối!"});
              res.json(money);
          });
          }else{
            Money.find({$and:[{_id_company: idcompany},{type:key}]},function(err,money){
            if(err) 
                res.json({success:false, message:"Lỗi kết nối!"});
              res.json(money);
          });
